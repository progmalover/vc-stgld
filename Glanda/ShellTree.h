...
public:
	...
	void	EnableImages();
	BOOL	GetSelectedFolderPath(CString &strFolderPath);
	bool	SearchTree(HTREEITEM treeNode, CString strSearchName, FindAttribs attr);
	...
	virtual LPITEMDATA AllocItemData();
	virtual void FreeItemData(LPITEMDATA pData);
...
// Implementation
public:
	HTREEITEM Locate(LPITEMIDLIST pidl);
	HTREEITEM Locate(LPCTSTR lpszPath);
	virtual ~CShellTree();

	// Generated message map functions
protected:
	HTREEITEM LocateITEMIDLIST(LPITEMIDLIST pidl);
	HTREEITEM LocatePath(LPCTSTR lpszPath);
