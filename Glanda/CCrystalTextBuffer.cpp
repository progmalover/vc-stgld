...
// static members
BOOL CCrystalTextBuffer::m_bCreateBackupFile = FALSE;
...
CCrystalTextBuffer::CCrystalTextBuffer()
{
	...
	m_bModifiedAfterLastCharInput = FALSE;
	m_bEnableUndo = TRUE;
}
...
BOOL CCrystalTextBuffer::InitNew(int nCrlfStyle /*= CRLF_STYLE_DOS*/, CString strDefault )
{
	ASSERT(! m_bInit);
	ASSERT(m_aLines.GetSize() == 0);
	ASSERT(nCrlfStyle >= 0 && nCrlfStyle <= 2);
	int nIndex = 0 ;
	while (	(nIndex = strDefault.Find("\r\n")) != -1 )
	{
		InsertLine(strDefault.Left(nIndex) );
		strDefault = strDefault.Right(strDefault.GetLength() - nIndex - 2 ) ;
	}
	
	//
	// Chen hao: the following line wrote by the author is error (will lost last empty line)
	// if the string is end with "\r\n"!!!
	//if (strDefault.GetLength() !=  0 )
		InsertLine(strDefault) ;
...
BOOL CCrystalTextBuffer::SaveToFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bClearModifiedFlag /*= TRUE*/)
{
	...
	__try
	{
		...
		// I add a retry code here because if some anti-virus software (fuck NAV2001)
		// enable auto-protection, even this basic API (MoveFile) may faile!
		int count = 0;
retry:
		if (! ::MoveFile(szTempFileName, pszFileName))
		{
			TRACE1("CCrystalTextBuffer::SaveToFile failed: GetLastError()=%d\n", GetLastError());
			if (count++ < 3)
			{
				Sleep(1);
				goto retry;
...
void CCrystalTextBuffer::GetText(int nStartLine, int nStartChar, int nEndLine, int nEndChar, CString &text, LPCTSTR pszCRLF /*= NULL*/)
{
	...
	UNREFERENCED_PARAMETER(pszCurPos);
}
...
BOOL CCrystalTextBuffer::InternalDeleteText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
	...
	//if (! m_bModified)
	// always set modified flag because we want to sync the CVisualEditView
	SetModified(TRUE);

	m_bModifiedAfterLastCharInput = TRUE;
...
BOOL CCrystalTextBuffer::CanUndo()
{
	...
	return m_bEnableUndo && !GetReadOnly() && m_nUndoPosition > 0;
}

BOOL CCrystalTextBuffer::CanRedo()
{
	ASSERT(m_nUndoPosition >= 0 && m_nUndoPosition <= m_aUndoBuf.GetSize());
	return m_bEnableUndo && !GetReadOnly() && m_nUndoPosition < m_aUndoBuf.GetSize();
}
...
void CCrystalTextBuffer::AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText)
{
	ASSERT(m_bEnableUndo);
	...
	if (nBufSize >= m_nUndoBufSize)
	{
		...
		// 
		// Modified by Chenhao 01-1-12 ÉÏÎç 11:13:26
		// Undo buffer overflowed, so set m_nSyncPosition to -1 to make the
		// modified flag m_bModified be TRUE always until save the file again
		//
		m_nSyncPosition = -1;
	}
	ASSERT(m_aUndoBuf.GetSize() < m_nUndoBufSize);
...
BOOL CCrystalTextBuffer::InsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
	// insert zero-length string may cause many assert fails
	// 2002-2-28 by Chen Hao
	...
	if (m_bEnableUndo)
	{
		...
		AddUndoRecord(TRUE, CPoint(nPos, nLine), CPoint(nEndChar, nEndLine), pszText);
		if (bGroupFlag)
			FlushUndoGroup();
	}
...
BOOL CCrystalTextBuffer::DeleteText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
	...
	if (m_bEnableUndo)
	{
		...
		AddUndoRecord(FALSE, CPoint(nStartChar, nStartLine), CPoint(nEndChar, nEndLine), sTextToDelete);
		if (bGroupFlag)
			FlushUndoGroup();
	}
...
void CCrystalTextBuffer::EmptyUndoBuffer()
{
	int nBufSize = m_aUndoBuf.GetSize();
...
void CCrystalTextBuffer::EnableUndo(BOOL bEnable)
{
	m_bEnableUndo = bEnable;
	if (!m_bEnableUndo && m_bInit)
		EmptyUndoBuffer();
}

CCrystalTextBuffer::SUndoRecord *CCrystalTextBuffer::GetLastUndoRecord()
{
	return &m_aUndoBuf.ElementAt(m_aUndoBuf.GetSize() - 1);
}