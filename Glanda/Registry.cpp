...
BOOL CRegistry::Read (LPCTSTR pszKey, CStringArray& scArray)
{
	...
	if(lReturn == ERROR_SUCCESS && dwType == REG_BINARY)
	{
		...
		//
		// The try catch is added by Chen hao on 2000-7-2.
		// Because the registry may be damaged
		//
		try
		{
			scArray.Serialize(ar);
		...
		{
			TRACE1("Error: Archive exception(%d)\n", e->m_cause);
#ifdef _DEBUG
			e->ReportError();
#endif
			e->Delete();
		...
		//
		// Comment out by Chen hao. Because we don't set the size of CStringArray 
		// befor call Read()
		//
