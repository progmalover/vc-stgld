                               ...
                               UINT nID, DWORD dwStyle)
{
    ...
    if (!CWnd::Create(wc.lpszClassName, lpszWindowName, dwStyle,
...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    int nLengthAvail = bHorz ? rc.Width() : rc.Height() - (arrSCBars.GetSize() - 1) * 2;
...
{
	// Copy from CCoolDialogbar by chenhao 11/17/1999
	...
	if (IsFloating())
	{
		...
		while (true)
		{
			...
			if ((pParent->GetStyle() & WS_CHILD) == 0)
				break;
		}
		if (m_bEnableSizing)
			pParent->ModifyStyle(MFS_4THICKFRAME, WS_THICKFRAME, SWP_FRAMECHANGED);
		else
			pParent->ModifyStyle(WS_THICKFRAME | MFS_4THICKFRAME, 0, SWP_FRAMECHANGED);
	}
	...
	if (IsFloating())
	{
		...
		{
		case HTTOPLEFT:
			m_szFloat.cx = max(window_rect.right - cursor_pt.x,
				m_szMin.cx) - m_cxEdge;
			m_szFloat.cy = max(window_rect.bottom - cyCaption - 
				cursor_pt.y,m_szMin.cy) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = min(cursor_pt.y,
				window_rect.bottom - cyCaption - m_szMin.cy) - 
				m_cxEdge;
			m_pDockContext->m_rectFrameDragHorz.left = min(cursor_pt.x,
				window_rect.right - m_szMin.cx) - 1;
			return m_szFloat;
			
		case HTTOPRIGHT:
			m_szFloat.cx = max(cursor_pt.x - window_rect.left,
				m_szMin.cx);
			m_szFloat.cy = max(window_rect.bottom - cyCaption - 
				cursor_pt.y,m_szMin.cy) - 1;
			m_pDockContext->m_rectFrameDragHorz.top = min(cursor_pt.y,
		...
		case HTBOTTOMLEFT:
			m_szFloat.cx = max(window_rect.right - cursor_pt.x,
				m_szMin.cx) - m_cxEdge;
			m_szFloat.cy = max(cursor_pt.y - window_rect.top - 
				cyCaption,m_szMin.cy);
			m_pDockContext->m_rectFrameDragHorz.left = min(cursor_pt.x,
				window_rect.right - m_szMin.cx) - 1;
			return m_szFloat;
...
void CSizingControlBar::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos)
{
    ...
    if (!IsFloating())
        if (lpwndpos->flags & SWP_SHOWWINDOW)
            m_bKeepSizeDocking = TRUE;
}

void CSizingControlBar::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
    ...
    if (!IsFloating())
	{
		// Added by Chen Hao, 2004-03-10
		// must update m_szHorz & m_szVert while changing from floating to docking state.
		// after docked, m_szHorz & m_szVert are upgraded in NegogtiateSpace()
		if ((lpwndpos->flags & SWP_SHOWWINDOW) && !IsWindowVisible())
		{
			if (IsHorzDocked())
				m_szHorz = CSize(lpwndpos->cx, lpwndpos->cy);
			else
				m_szVert = CSize(lpwndpos->cx, lpwndpos->cy);
		}

		GetWindowRect(&m_pDockContext->m_rectMRUDockPos);
		GetParent()->ScreenToClient(&m_pDockContext->m_rectMRUDockPos);
	}
	else
	{
		if (m_pDockContext && m_pDockContext->m_rectMRUDockPos.IsRectEmpty())
		{
			GetWindowRect(&m_pDockContext->m_rectMRUDockPos);
			GetParent()->ScreenToClient(&m_pDockContext->m_rectMRUDockPos);
		}
                                     ...
                                     NCCALCSIZE_PARAMS FAR* lpncsp) 
{
    ...
    rcClient.DeflateRect(m_cxBorder, m_cxBorder);
...
{
	if ((m_dwSCBStyle & SCBS_GRIPPER) == 0)
		return;
...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
	...
	GetCursorPos(&m_ptOld);				//Modified by Chen Hao, 2002-9-11
                                    ...
                                    CRect& rcEdge)
{
	...
	int nSize = m_cxEdge + m_cxBorder;;
	
	BOOL bSideEdge = (!bHorz && (nHitTest == HTTOP || nHitTest == HTBOTTOM)) || 
		(bHorz && (nHitTest == HTLEFT || nHitTest == HTRIGHT));

	switch (nHitTest)
    {
    case HTLEFT:
        if (!(m_dwSCBStyle & SCBS_EDGELEFT)) return FALSE;
		rcEdge.right = rcEdge.left + (bSideEdge ? m_cxEdge : nSize);
        rcEdge.DeflateRect(0, bHorz ? nSize: 0);
        break;

    case HTTOP:
        if (!(m_dwSCBStyle & SCBS_EDGETOP)) return FALSE;
        rcEdge.bottom = rcEdge.top + (bSideEdge ? m_cxEdge : nSize);
        rcEdge.DeflateRect(bHorz ? 0 : nSize, 0);
        break;

    case HTRIGHT:
        if (!(m_dwSCBStyle & SCBS_EDGERIGHT)) return FALSE;
        rcEdge.left = rcEdge.right - (bSideEdge ? m_cxEdge : nSize);
        rcEdge.DeflateRect(0, bHorz ? nSize: 0);
        break;

    case HTBOTTOM:
        if (!(m_dwSCBStyle & SCBS_EDGEBOTTOM)) return FALSE;
        rcEdge.top = rcEdge.bottom - (bSideEdge ? m_cxEdge : nSize);
        rcEdge.DeflateRect(bHorz ? 0 : nSize, 0);
        break;
...
//	TRACE1("Enter (%s) --------------------\n", (LPCTSTR)strText);
//	TRACE2("Horz: %d, %d\n", m_szHorz.cx, m_szHorz.cy);
//	TRACE2("Vert:  %d, %d\n", m_szVert.cx, m_szVert.cy);
//#endif
	...
	if (nCount == 1)
	{
		if (nDelta != 0)
		{
			...
			if (bHorz)
			{
				pBar->m_szHorz.cx += nDelta;
				if (pBar->m_szHorz.cx < pBar->m_szMin.cx)
					pBar->m_szHorz.cx = pBar->m_szMin.cx;
			}
			else
			{
				pBar->m_szVert.cy += nDelta;
				if (pBar->m_szVert.cy < pBar->m_szVert.cy)
					pBar->m_szVert.cy = pBar->m_szVert.cy;
			}
		}
	}
	else
	{
		if (nDelta > 0)
		{
			...
			{
				pBar = arrSCBars[i];
				if (!pBar->m_bKeepSize && !pBar->m_bKeepSizeDocking)
					nCount2++;
			}

			if (nCount2 > 0)
			{
				int nDeltaAve = nDelta / nCount2;
				int nRest = nDelta % nCount2;
				for (i = 0; i < nCount; i++)
				{
					pBar = arrSCBars[i];
					if (!pBar->m_bKeepSize && !pBar->m_bKeepSizeDocking)
					{
						if (bHorz)
							pBar->m_szHorz.cx += nDeltaAve + (i == nCount2 - 1 ? nRest : 0);
						else
							pBar->m_szVert.cy += nDeltaAve + (i == nCount2 - 1 ? nRest : 0);
					}
				}
			}

			// if no not KeepSize bar found, resize the last KeepSize bar.
			if (nCount2 == 0)
			{
		...
		if (nDelta < 0)
		{
			// try to compact all m_bKeepSIze bars
			for (i = 0; i < nCount; i++)
			{
				pBar = arrSCBars[i];
				if (pBar->m_bKeepSize)
				{
					if (bHorz)
					{
						int nDelta2 = pBar->m_szMin.cx - pBar->m_szHorz.cx;
						if (nDelta2 < 0)	// larger than min size
						{
							nDelta2 = max(nDelta2, nDelta);
							pBar->m_szHorz.cx += nDelta2;
							nDelta -= nDelta2;
						}
					}
					else
					{
						int nDelta2 = pBar->m_szMin.cy - pBar->m_szVert.cy;
						if (nDelta2 < 0)	// larger than min size
						{
							nDelta2 = max(nDelta2, nDelta);
							pBar->m_szVert.cy += nDelta2;
							nDelta -= nDelta2;
						}
					}
				}
			...
			// after resized all m_bKeeySize bars, if nActualLength is still too large, 
			// then try to resize the not m_bKeepSize bars.
			if (nDelta < 0)
			{
				int nCount2 = 0;
				for (i = 0; i < nCount; i++)
					...
					for (i = 0; i < nCount; i++)
					{
						...
						if (!pBar->m_bKeepSize && !pBar->m_bKeepSizeDocking)
						{
							if (bHorz)
							{
								...
								if (nDelta2 < 0)	// larger than min size
								{
									nDelta2 = max(nDelta2, nDeltaAve + (i == nCount2 - 1 ? nRest : 0));
									pBar->m_szHorz.cx += nDelta2;
									nDelta -= nDelta2;
								}

								nRest += (nDeltaAve - nDelta2);
							}
							else
							{
								int nDelta2 = pBar->m_szMin.cy - pBar->m_szVert.cy;
	...
	{
		pBar = arrSCBars[i];
		//pBar->GetWindowRect(&pBar->m_pDockContext->m_rectMRUDockPos);
		pBar->m_bKeepSizeDocking = FALSE;
	}
...
void CSizingControlBar::ToggleDocking(void)
{
	if (m_pDockBar != NULL)
	{
		ASSERT(m_pDockContext != NULL);
...
void CSizingControlBar::EnableDocking(DWORD dwDockStyle)
{
	...
	if (m_pDockContext == NULL)
		m_pDockContext = new CDockContextEx((CControlBar *)this);
...
BOOL CSizingControlBar::SetStatusText(INT_PTR nHit)
{
	// to disable default statusbar message
	return FALSE;
}
...
BOOL CSizingControlBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	if (nHitTest == HTLEFT || nHitTest == HTRIGHT)
		nID = AFX_IDC_HSPLITBAR;
	else if (nHitTest == HTTOP || nHitTest == HTBOTTOM)
		nID = AFX_IDC_VSPLITBAR;
	else
...
LRESULT CSizingControlBar::OnSetText(WPARAM wp, LPARAM lp)
{
	LRESULT ret = Default();
	if (ret != 0)
	{
		if (IsFloating())
		{
			CWnd *pFrame = GetParent()->GetParent();
			if (pFrame && pFrame->IsKindOf(RUNTIME_CLASS(CMiniFrameWnd)))
				pFrame->SetWindowText((LPCTSTR)lp);
		}
	}
	return ret;
}
