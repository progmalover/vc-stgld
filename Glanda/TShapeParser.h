...
protected:
	...
	virtual void OnGetBound(const GGRECT &bound);
private:	
...
public:
	CTShapeParser();
	virtual ~CTShapeParser(void);

	virtual bool Parse(int dataLen, unsigned char *data);
    virtual bool Parse(gldShape *gshape);
	gld_shape *_M_shape;
private:
	gld_draw_obj _M_cur_do;
	gld_a_point _M_cur_pos;
	gld_edge _M_prev_edge;
