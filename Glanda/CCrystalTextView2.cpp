...
#define CRYSTAL_TIMER_DRAGSEL_INTERVAL			80

#define CRYSTAL_TIMER_DELAYUPDATECARET			1002
#define CRYSTAL_TIMER_DELAYUPDATECARET_INTERVAL	1
...
void CCrystalTextView::MoveLeft(BOOL bSelect)
{
	...
	LPCTSTR pszLine = GetLineChars(m_ptCursorPos.y);
	if (m_ptCursorPos.x > 0)
		if (__ismbstrail(pszLine, m_ptCursorPos.x))
			m_ptCursorPos.x --;
...
void CCrystalTextView::MoveRight(BOOL bSelect)
{
	...
	LPCTSTR pszLine = GetLineChars(m_ptCursorPos.y);
	if (m_ptCursorPos.x > 0 && m_ptCursorPos.x < GetLineLength(m_ptCursorPos.y))
		if (__ismbstrail(pszLine, m_ptCursorPos.x))
			m_ptCursorPos.x ++;
...
void CCrystalTextView::MoveWordLeft(BOOL bSelect)
{
	...
	while (nPos > 0 && __isspace(pszChars[nPos - 1]))
		nPos --;

	if (nPos > 0)
	{
		if (nPos > 1 && __ismbstrail(pszChars, nPos - 1))
		{
			nPos -= 2;
		}
		else
		{
			nPos --;
			if (__isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
			{
				while (nPos > 0 && (__isalnum(pszChars[nPos - 1]) || pszChars[nPos - 1] == _T('_')))
					nPos --;
			}
			else
			{
				while (nPos > 0 && ! __isalnum(pszChars[nPos - 1]) && pszChars[nPos - 1] != _T('_') && ! __isspace(pszChars[nPos - 1]) && !__ismbstrail(pszChars, nPos - 1))
					nPos --;
			}
		}
	}
...
void CCrystalTextView::MoveWordRight(BOOL bSelect)
{
	...
	if (nPos < nLength - 1 && __ismbslead(pszChars, nPos))
	{
		nPos += 2;
	}	// mbs char
	else if (__isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
	{
		while (nPos < nLength && __isalnum(pszChars[nPos]) || pszChars[nPos] == _T('_'))
			nPos ++;
	}	// part of a word
	else
	{
		while (nPos < nLength && ! __isalnum(pszChars[nPos]) && pszChars[nPos] != _T('_') && ! __isspace(pszChars[nPos]) && !__ismbslead(pszChars, nPos))
			nPos ++;
	}	// space, punctuation(.,+=...)

	while (nPos < nLength && __isspace(pszChars[nPos]))
		nPos ++;
...
void CCrystalTextView::MoveHome(BOOL bSelect)
{
	...
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	int nHomePos = 0;
	while (nHomePos < nLength && __isspace(pszChars[nHomePos]))
		nHomePos ++;
...
{
	int nNewTopLine = m_nTopLine + GetScreenLines() - 1;
	if (nNewTopLine > GetLineCount() - GetScreenLines())
		nNewTopLine = GetLineCount() - GetScreenLines();
	if (nNewTopLine < 0)
		nNewTopLine = 0;
...
{
	if (m_nTopLine < GetLineCount() - GetScreenLines())
	{
		m_nTopLine ++;
		ScrollWindow(0, - GetLineHeight());
		RecalcVertScrollBar(TRUE);
...
{
	if (m_nOffsetChar < GetMaxLineLength() - GetScreenChars())
	{
		m_nOffsetChar ++;
		CRect rcScroll;
		GetClientRect(&rcScroll);
...
CPoint CCrystalTextView::WordToRight(CPoint pt)
{
	...
	while (pt.x < nLength)
	{
		if (! __isalnum(pszChars[pt.x]) && pszChars[pt.x] != _T('_'))
			break;
		pt.x ++;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
CPoint CCrystalTextView::WordToLeft(CPoint pt)
{
	...
	while (pt.x > 0)
	{
		if (! __isalnum(pszChars[pt.x - 1]) && pszChars[pt.x - 1] != _T('_'))
			break;
		pt.x --;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
void CCrystalTextView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (point.x < GetMarginWidth())
	{
		...
		else
		{
			...
			SetCapture();
			m_nDragSelTimer = SetTimer(CRYSTAL_TIMER_DRAGSEL, CRYSTAL_TIMER_DRAGSEL_INTERVAL, NULL);
...
void CCrystalTextView::DelayUpdateCaret()
{
	SetTimer(CRYSTAL_TIMER_DELAYUPDATECARET, CRYSTAL_TIMER_DELAYUPDATECARET_INTERVAL, NULL);
}
...
void CCrystalTextView::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent == CRYSTAL_TIMER_DRAGSEL)
	{
		...
		if (nNewTopLine > nLineCount - GetScreenLines())
			nNewTopLine = nLineCount - GetScreenLines();
		if (nNewTopLine < 0)
			nNewTopLine = 0;
		...
		if (nNewOffsetChar > nMaxLineLength - GetScreenChars())
			nNewOffsetChar = nMaxLineLength - GetScreenChars();
		if (nNewOffsetChar < 0)
			nNewOffsetChar = 0;
	...
	else if (nIDEvent == CRYSTAL_TIMER_DELAYUPDATECARET)
	{
		KillTimer(CRYSTAL_TIMER_DELAYUPDATECARET);
		UpdateCaret();
	}
}
...
BOOL CCrystalTextView::PutToClipboard(LPCTSTR pszText)
{
	...
	if (OpenClipboard())
	{
		...
		{
			// CF_UNICODETEXT : Windows NT/2000/XP compatible
			//USES_CONVERSION;
			LPWSTR pwszText = MY_A2W(pszText);
			hData = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, (wcslen(pwszText) + 1) * 2);
			if (hData)
			{
				LPWSTR pwszData = (LPWSTR)::GlobalLock(hData);
				wcscpy(pwszData, pwszText);
				GlobalUnlock(hData);
