  ...
  else
  {
    ...
    if(!strText.IsEmpty())
    {
      ...
      if(!(lpDIS->itemState & ODS_GRAYED))
      {
        ...
        if(bIsMenuBar)
        {
          MenuDrawText(pDC->m_hDC,leftStr,-1,rectt,nFormat);
          if(tablocr!=-1)
          {
            MenuDrawText(pDC->m_hDC,rightStr,-1,rectt,nFormatr);
          }
        }
        else
        {
