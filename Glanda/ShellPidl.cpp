...
LPITEMIDLIST CShellPidl::CopyITEMID(LPMALLOC lpMalloc, LPITEMIDLIST lpi)
{
   ...
   int size = GetSize(lpi);
   lpiTemp=(LPITEMIDLIST)lpMalloc->Alloc(size);
   CopyMemory((PVOID)lpiTemp, (CONST VOID *)lpi, size);
...
*
****************************************************************************/
LPITEMIDLIST CShellPidl::CopyITEMID(LPITEMIDLIST lpi)
{
   LPITEMIDLIST lpiTemp;
   ...
   //CopyMemory((PVOID)lpiTemp, (CONST VOID *)lpi, lpi->mkid.cb+sizeof(lpi->mkid.cb));
   int size = GetSize(lpi);
   lpiTemp=(LPITEMIDLIST)CoTaskMemAlloc(size);
   CopyMemory((PVOID)lpiTemp, (CONST VOID *)lpi, size);
     ...
     LPITEMIDLIST  lpi, LPPOINT lppt)
{
	...
	TRACE0("Will failed for desktop.\n");
