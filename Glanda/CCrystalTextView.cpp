...
BEGIN_MESSAGE_MAP(CCrystalTextView, CView)
	...
	ON_COMMAND(ID_EDIT_NEXT_BOOKMARK, OnEditNextBookmark)
	ON_COMMAND(ID_EDIT_PREV_BOOKMARK, OnEditPrevBookmark)
	ON_COMMAND(ID_EDIT_CLEAR_BOOKMARKS, OnEditClearBookmarks)
	//}}AFX_MSG_MAP
	...
	//ON_COMMAND(ID_EDIT_FIND_NEXT_SELECTED, OnEditFindNextSelected)
	ON_WM_MOUSEACTIVATE()
END_MESSAGE_MAP()
									 ...
									 LPCTSTR pszChars, int nOffset, int nCount)
{
	...
	if (nCount > 0)
	{
		...
		if (nWidth > 0)
		{
			...
			////////////////////////////////////////////////////////////////
			// Fixed by Chen hao, to displaying right DBCS correctly
			// 2001-10-15
			//int nCountFit = nWidth / nCharWidth + 1;
			int nCountFit = nWidth / nCharWidth + 2;
			if (nCount > nCountFit)
				nCount = nCountFit;
#ifdef _DEBUG
									 ...
									 LPCTSTR pszChars, int nOffset, int nCount, CPoint ptTextPos)
{
	...
	UNREFERENCED_PARAMETER(nColorIndex);
}
					...
					COLORREF &crText, BOOL &bDrawWhitespace)
{
	...
	{
		crBkgnd = RGB(0, 0, 128) ;/*m_bFocused ? ::GetSysColor(COLOR_HIGHLIGHT) : RGB(0x7a, 0x96, 0xdf)*/;
		return;
	}
...
void CCrystalTextView::DrawSingleLine(CDC *pdc, const CRect &rc, int nLineIndex)
{
	...
	//#pragma message(__FILELINE__ "Warning: Temporarily comment on 12/1/2001. No error found yet.")
	//pdc->FillSolidRect(rc, GetColor(COLORINDEX_WHITESPACE));
	...
	if (nBlocks > 0)
	{
		...
		for (int I = 0; I < nBlocks - 1; I ++)
		{
			...
			/*
			if (pBuf[I].m_nColorIndex == COLORINDEX_AS_STARTTAG || pBuf[I].m_nColorIndex == COLORINDEX_AS_ENDTAG)
				pdc->SetBkColor(GetColor(pBuf[I].m_nColorIndex + 1));
			else
			*/
	...
	UNREFERENCED_PARAMETER(bColorSet);
}
...
COLORREF CCrystalTextView::GetColor(int nColorIndex)
{
	...
	case COLORINDEX_BKGND:
		return ::GetSysColor(bEnabled ? COLOR_WINDOW : COLOR_3DFACE);
	case COLORINDEX_NORMALTEXT:
		return ::GetSysColor(bEnabled ? COLOR_WINDOWTEXT : COLOR_GRAYTEXT);
	case COLORINDEX_SELMARGIN:
		return ::GetSysColor(COLOR_SCROLLBAR);
	case COLORINDEX_PREPROCESSOR:
		return bEnabled ? RGB(0, 128, 192) : ::GetSysColor(COLOR_GRAYTEXT);
	case COLORINDEX_COMMENT:
		return bEnabled ? RGB(128, 128, 128) : ::GetSysColor(COLOR_GRAYTEXT);
	case COLORINDEX_KEYWORD:
		return bEnabled ? RGB(0, 0, 255) : ::GetSysColor(COLOR_GRAYTEXT);
	case COLORINDEX_SELBKGND:
		return ::GetSysColor(bEnabled  ? (m_bFocused ? COLOR_HIGHLIGHT : COLOR_INACTIVECAPTION)  : COLOR_3DFACE);
	case COLORINDEX_SELTEXT:
		return ::GetSysColor(bEnabled ? COLOR_HIGHLIGHTTEXT : COLOR_GRAYTEXT);
	case COLORINDEX_STRING:
		return bEnabled ? RGB(0, 128, 128) : ::GetSysColor(COLOR_GRAYTEXT);
	}
	return ::GetSysColor(bEnabled ? COLOR_WINDOWTEXT : COLOR_GRAYTEXT);
}
...
void CCrystalTextView::DrawMargin(CDC *pdc, const CRect &rect, int nLineIndex)
{
	...
	{
		CBrush br;
		br.CreateSolidBrush(GetColor(COLORINDEX_BKGND));
		pdc->FillRect(&rect, &br);
		return;
	}

	CRect rcFirst(rect.left, rect.top, rect.right - 5 - 1, rect.bottom);
	CRect rcSecond(rect.right - 5, rect.top, rect.right, rect.bottom);

	//COLORREF crBkOld, crTextOld;
	//crBkOld = pdc->SetBkColor(GetColor(COLORINDEX_BKGND));
	//crTextOld = pdc->SetTextColor(GetColor(COLORINDEX_SELMARGIN));
	//pdc->FillRect(rcFirst, pdc->GetHalftoneBrush());
	//pdc->SetBkColor(crBkOld);
	//pdc->SetTextColor(crTextOld);
	pdc->FillSolidRect(rcFirst, ::GetSysColor(COLOR_3DFACE));

	CPen pen(PS_SOLID, 1, ::GetSysColor(COLOR_3DSHADOW));
	CPen *pOldPen = pdc->SelectObject(&pen);
	pdc->MoveTo(rcFirst.right, rcFirst.top);
	pdc->LineTo(rcFirst.right, rcFirst.bottom);
	pdc->SelectObject(pOldPen);

	pdc->FillSolidRect(rcSecond, GetColor(COLORINDEX_BKGND));
	...
	if (nLineIndex >= 0)
	{
		...
		{
			TCHAR szNum[11];	// UINT_MAX = 4294967295
			itoa(nLineIndex + 1, szNum, 10);
			CFont *pOldFont = pdc->SelectObject(&m_fntLineNum);
			COLORREF crText = pdc->SetTextColor(GetColor(COLORINDEX_NORMALTEXT));
			COLORREF nBkMode = pdc->SetBkMode(TRANSPARENT);
			CRect rcText = rcFirst;
			rcText.left--;
			pdc->DrawText(szNum, lstrlen(szNum), (LPRECT)&rcText, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	...
	if (nImageIndex >= 0)
	{
		...
		{ 
			...
			VERIFY(m_pIcons->Create(IDR_MARGIN_ICONS, 16, 1, RGB(255, 255, 255)));
		}
		CPoint pt(rcFirst.left + (rcFirst.Width() - 16 ) / 2, rcFirst.top + (rcFirst.Height() - 16) / 2);
...
// PARAMETERS:		
// RETURN VALUE:	
// NOTE:			This function is not virtual original. I modified it because
//					I want to track the caret position change in derived class
//
void CCrystalTextView::UpdateCaret()
{
		...
		CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x) >= m_nOffsetChar)
	{
		...
		// New code. fix to make it more similar to VC
		// 2001-07-08
		CPoint point = TextToClient(m_ptCursorPos);
		point.x = max(GetMarginWidth(), point.x - 1);
		SetCaretPos(point);
...
void CCrystalTextView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	if (nNewTopLine > nLineCount - nPageLines)
...
//#define SMOOTH_SCROLL
		...
		{
			int nLineHeight = GetLineHeight();
			//int nStep = nScrollLines << 1;
			//for (int i = 0; i < nLineHeight / 2; i++)
			//	ScrollWindow(0, nStep);
			//if (nLineHeight % 2 != 0)
	...
	UNREFERENCED_PARAMETER(bUpdateSiblings);
}
...
void CCrystalTextView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	if (nNewOffset > nMaxLineLength - nPageChars)
...
CPoint CCrystalTextView::ClientToText(const CPoint &point)
{
	...
	if (pszLine != NULL)
		if (nIndex > 0 && nIndex < nLength)
			if (__ismbstrail(pszLine, nIndex))
				pt.x ++;
...
{
	return 0;
	UNREFERENCED_PARAMETER(nActualItems);
	UNREFERENCED_PARAMETER(pBuf);
	UNREFERENCED_PARAMETER(nLineIndex);
								  ...
								  DWORD dwFlags, int nLineIndex /*= -1*/)
{
	...
	if (pContext != NULL)
	{
		...
		#pragma message("this line is temporarily comment out.")
		//UpdateCaret();
	}
...
int CCrystalTextView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	/////////////////////////////////////////////////////
	// Edit by Chenhao 99-11-18 下午 02:29:15

	CDC *pDC = GetDC();

	m_lfBaseFont.lfHeight = -MulDiv(12, pDC->GetDeviceCaps(LOGPIXELSY), 72);
	...
	// Font used by line number
	m_fntLineNum.CreatePointFont(7 * 10, "Small Fonts", pDC);
...
void CCrystalTextView::SetCursorPosSimply(const CPoint &point)
{
	ASSERT_VALIDTEXTPOS(point);
	if ((point.y >= 0 && point.y < GetLineCount() && point.x >= 0 && point.x <= GetLineLength(point.y)))
	{
		SetAnchor(point);
...
void CCrystalTextView::SetFont(const LOGFONT &lf)
{
	...
	if (::IsWindow(m_hWnd))
	{
		RecalcVertScrollBar();
		RecalcHorzScrollBar();
		// This judgement is added by Chen hao, 3/10/2000. 
		// To avoid property grid contents to be changed when HTMLView is not the active view.
		if (IsWindowVisible())
		{
			Invalidate();
			UpdateCaret();
...
void CCrystalTextView::OnUpdateIndicatorPosition(CCmdUI* pCmdUI)
{
	...
	ASSERT_VALIDTEXTPOS(m_ptCursorPos);
	// 
	// Modified by Chenhao 01-2-7 下午 03:15:33
	// must calculate TAB size
	int x = CalculateActualOffset(m_ptCursorPos.y, m_ptCursorPos.x);
	CString stat;
	//stat.Format(IDS_LN_COL, m_ptCursorPos.y + 1, x + 1);
	pCmdUI->SetText(stat);
}
...
void CCrystalTextView::OnUpdateIndicatorCRLF(CCmdUI* pCmdUI)
{
	...
	{
		// 
		// Modified by Chenhao 01-2-14 下午 05:02:33
		// MAC和UNIX是故意反过来的，因为原来的分析代码有问题
...
static int FindStringHelper(LPCTSTR pszFindWhere, LPCTSTR pszFindWhat, BOOL bWholeWord)
{
	...
	for (;;)
	{
		...
		if (pszPos > pszFindWhere && (__isalnum(pszPos[-1]) || pszPos[-1] == _T('_')))
		{
			nCur += (pszPos - pszFindWhere);
			pszFindWhere = pszPos + 1;
			continue;
		}
		if (__isalnum(pszPos[nLength]) || pszPos[nLength] == _T('_'))
		{
...
void CCrystalTextView::GoBookmark(int index)
{

}

void CCrystalTextView::OnEditToggleBookmark() 
{
	...
	if (m_pTextBuffer != NULL)
	{
		...
		m_pTextBuffer->SetLineFlag(m_ptCursorPos.y, LF_BOOKMARK, (dwFlags & LF_BOOKMARK) == 0, FALSE);
	}
}

void CCrystalTextView::OnEditNextBookmark() 
{
	// TODO: Add your command handler code here
	...
	for (int count = 0; count < nLines; count ++)
	{
		...
		if ((GetLineFlags(i) & LF_BOOKMARK) != 0)
		{
			CPoint pt(0, i);
...
void CCrystalTextView::OnEditPrevBookmark() 
{
	// TODO: Add your command handler code here
...
void CCrystalTextView::OnEditClearBookmarks() 
{
	// TODO: Add your command handler code here
	...
	if (m_pTextBuffer != NULL)
	{
		...
		{
			DWORD dwFlags = GetLineFlags(i);
			if (dwFlags & LF_BOOKMARK)
			{
				m_pTextBuffer->SetLineFlag(i, LF_BOOKMARK, FALSE, FALSE);
			}
		}
	}
}

void CCrystalTextView::SetAutoIndent(BOOL bAutoIndent)
{
	m_bAutoIndent = bAutoIndent;
}

void CCrystalTextView::SetFont(LPCTSTR lpszFontFace, int nFontSize)
{
	...
	lstrcpy(lf.lfFaceName, lpszFontFace);
	lf.lfHeight = -MulDiv(nFontSize, pDC->GetDeviceCaps(LOGPIXELSY), 72);
...
BOOL CCrystalTextView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	{
		int nTimes = abs(zDelta / WHEEL_DELTA);
...
void CCrystalTextView::OnEditFindNextSelected()
{
	...
	if (ptStart == ptEnd)
	{
		...
		HighlightText(ptStart, ptEnd.x - ptStart.x);
	}
	...
	if (m_bLastSearch)
	{
		dwFlags = m_dwLastSearchFlags;
	}
	else
	{
		if (AfxGetApp()->GetProfileInt(REG_REPLACE_SUBKEY, REG_MATCH_CASE, FALSE))
			m_dwLastSearchFlags |= FIND_MATCH_CASE;
		if (AfxGetApp()->GetProfileInt(REG_REPLACE_SUBKEY, REG_WHOLE_WORD, FALSE))
			m_dwLastSearchFlags |= FIND_WHOLE_WORD;

		m_bLastSearch = TRUE;
		m_dwLastSearchFlags = dwFlags;
	}

	if (m_pszLastFindWhat)
		free(m_pszLastFindWhat);
	m_pszLastFindWhat = _tcsdup(strText);

	if (FindText(strText, m_ptCursorPos, dwFlags, TRUE, &ptFound))
		HighlightText(ptFound, strText.GetLength());
	else
		MessageBeep(MB_ICONQUESTION);
}

int CCrystalTextView::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
	// TODO: Add your message handler code here and/or call default

	OnActivateView(TRUE, this, this);

	if (!GetParent()->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
		return CWnd::OnMouseActivate(pDesktopWnd, nHitTest, message);
	else
		return CView::OnMouseActivate(pDesktopWnd, nHitTest, message);
}

void CCrystalTextView::OnSelectChange()
{
}